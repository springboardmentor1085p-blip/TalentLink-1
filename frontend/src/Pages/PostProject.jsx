      ...
      {open && (
        <div className="absolute z-10 w-full mt-2 bg-white border rounded-lg shadow max-h-80 overflow-auto">
          <div className="sticky top-0 bg-white border-b p-2 flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => { onSelect(''); setOpen(false); setTerm(''); }}
          ...
          <div className="p-2">
            {filtered.map(group => (
              <div key={group.group} className="mb-2">
                <div className="text-xs font-bold text-gray-500 px-2 py-1">{group.group}</div>
                {group.items.map(item => (
                  <button
                    key={item}
                    type="button"
                    onClick={() => { onSelect(item); setOpen(false); setTerm(''); }}
                    className={`w-full text-left px-3 py-2 rounded ${selected === item ? 'bg-purple-100 text-purple-700' : 'hover:bg-purple-50'}`}
                  >
                    {item}
                  </button>
                ))}
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      (formData.skills || []).forEach(s => {
        ...
        if (id != null) fd.append('skill_ids', id);
      });
      ...
      await projectsAPI.createMultipart(fd);
      ...
      <div className="container mx-auto px-4 max-w-4xl">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-8 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* Job Type */}
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Engagement Type</label>
              <select
            ...
            {formData.job_type === 'fixed' ? (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Fixed Payment (₹)</label>
                <input
                  type="number"
                  min="1"
                  value={formData.fixed_payment}
                  onChange={e => updateField('fixed_payment', e.target.value)}
              ...
              <>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Hourly Min (₹/hr)</label>
                  <input
                    type="number"
                    min="1"
                    value={formData.hourly_min}
                    onChange={e => updateField('hourly_min', e.target.value)}
                    placeholder="e.g., 500"
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Hourly Max (₹/hr)</label>
                  <input
                    type="number"
                    min="1"
                    value={formData.hourly_max}
                    onChange={e => updateField('hourly_max', e.target.value)}
            ...
            <div>
              ...
              <select
                ...
                onChange={e => updateField('duration_estimate', e.target.value)}
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
              >
            ...
            <div className="col-span-1 md:col-span-2">
              ...
              <p className="mt-2 text-xs text-gray-500">
                Pick the most relevant category. You can refine in filters later.
              </p>
            </div>
            ...
            <div className="col-span-1 md:col-span-2">
              ...
              <p className="mt-2 text-xs text-gray-500">
                Supported: common docs/images. Larger files may take longer to upload.
              </p>
            </div>
          </div>
